...
public class FreshAirDataFormate {
    ...
    public static Map<String, Object> formateDataForEchar(List<Map<String, Object>> devicepartTimeData, String code) {
        // 查找设备参数Id
        List<Map<String, Object>> categoryParameter = new ArrayList<Map<String, Object>>();
        Map<String, Object> codeData = new HashMap<String, Object>();
        Double data_max = Double.valueOf(-10000);
        Double data_min = Double.valueOf(10000);
        Double data_avg = Double.valueOf(0);
        Double data_sum = Double.valueOf(0);
        switch (code) {
        case "wendu" : {
            codeData.put("name", "温度");
            codeData.put("unit", "℃");
            break;
        }
        case "shidu" : {
            codeData.put("name", "湿度");
            codeData.put("unit", "％");
            break;
        }
        case "pm" : {
            codeData.put("name", "PM2.5");
            codeData.put("unit", "μg/m³");
            break;
        }
        case "voc" : {
            codeData.put("name", "VOC");
            codeData.put("unit", "g/L");
            break;
        }
        case "co2" : {
            codeData.put("name", "CO2");
            codeData.put("unit", "ppm");
            break;
        }
        default : {
            return null;
        ...
        if (CollectionUtil.isNotEmpty(devicepartTimeData)) {
            for (Map<String, Object> deviceData : devicepartTimeData) {
                dateList.add(Long.parseLong(deviceData.get("recordTime").toString()));
                Double wendu_val = (Double) deviceData.get(code);
                wendu_val_list.add(wendu_val);
                data_min = data_min > wendu_val ? wendu_val : data_min;
                data_max = data_max > wendu_val ? data_max : wendu_val;
                data_sum += wendu_val;
            }
